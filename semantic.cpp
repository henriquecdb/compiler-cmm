...
string SemanticAnalyzer::extractId(ASTNode *node)
{
    ...
    if (startsWith(node->label, "ID."))
        return suffixAfterDot(node->label);
    return node->label;
}

string SemanticAnalyzer::locationOf(ASTNode *node) const
{
    if (!node || node->linha < 0 || node->coluna < 0)
        return "";
...
TypeInfo SemanticAnalyzer::typeFromNode(ASTNode *node) const
{
    ...
    if (startsWith(node->label, "TYPE."))
        return typeFromName(suffixAfterDot(node->label));
    return TypeInfo::Error();
}
...
bool SemanticAnalyzer::compatibleAssignment(const TypeInfo &target, const TypeInfo &value) const
{
    ...
    if (target.isArray || value.isArray)
        return target.isArray == value.isArray && target.name == value.name;
...
{
    if (!root || root->label != "PROGRAM" || root->children.empty())
        return;
...
void SemanticAnalyzer::collectTypeDecl(ASTNode *node)
{
    ...
    if (!symbols.declare(info))
        reportError("erro semantico: tipo '" + alias + "' ja declarado no escopo atual");
}
...
void SemanticAnalyzer::collectFunctionSignature(ASTNode *node)
{
    ...
    if (node->children[2])
    {
        ...
        {
            if (!param || param->label != "PARAM" || param->children.size() < 2)
                continue;
            TypeInfo paramType = typeFromNode(param->children[0]);
            if (param->children.size() > 2 && param->children[2])
            {
                if (param->children[2]->children.empty())
                    paramType = TypeInfo::ArrayOf(paramType);
                else if (param->children[2]->children[0] && startsWith(param->children[2]->children[0]->label, "NUMBER."))
                    paramType = TypeInfo::ArrayOf(paramType, stoi(suffixAfterDot(param->children[2]->children[0]->label)));
                else
                    paramType = TypeInfo::ArrayOf(paramType);
            }
    ...
    if (!symbols.declare(info))
        reportError("erro semantico: funcao '" + name + "' ja declarada no escopo atual");
}
...
void SemanticAnalyzer::analyzeDecl(ASTNode *node)
{
    if (!node)
        return;
    if (node->label == "TYPE_DECL")
        analyzeTypeDecl(node);
    else if (node->label == "VAR_DECL_LIST")
        analyzeVarDeclList(node, true);
    else if (node->label == "FUNCTION_DECL")
...
void SemanticAnalyzer::analyzeVarDeclList(ASTNode *node, bool emitDeclarations)
{
    ...
    if (node->label != "VAR_DECL_LIST" || node->children.empty())
        return;

    TypeInfo base = typeFromNode(node->children[0]->children[0]);
    for (auto *entry : node->children)
    {
        if (!entry || entry->children.size() < 2)
            continue;
...
{
    if (!node || node->label != "FORMAL_LIST")
        return;
    for (auto *param : node->children)
    {
        ...
        {
            if (param->children[2]->children.empty())
                info.type = TypeInfo::ArrayOf(info.type);
            else if (param->children[2]->children[0] && startsWith(param->children[2]->children[0]->label, "NUMBER."))
                info.type = TypeInfo::ArrayOf(info.type, stoi(suffixAfterDot(param->children[2]->children[0]->label)));
            else
                info.type = TypeInfo::ArrayOf(info.type);
        }
...
void SemanticAnalyzer::analyzeFunctionDecl(ASTNode *node)
{
    ...
    for (auto &param : params)
    {
        if (!symbols.declare(param))
            reportError("erro semantico: parametro '" + param.name + "' ja declarado na funcao '" + name + "'");
        else
            emitDeclaration(param);
    }
    if (node->children[3])
        analyzeVarDeclList(node->children[3], true);
    if (node->children[4])
        analyzeStmtList(node->children[4]);
...
void SemanticAnalyzer::analyzeStmt(ASTNode *node)
{
    ...
    if (node->label == "BREAK")
    {
        if (!loopExitLabels.empty())
            emitter.emit("goto " + loopExitLabels.back());
        else
            reportError(node, "erro semantico: break fora de laco");
        return;
    }
    if (node->label == "PRINT")
    {
    ...
    if (node->label == "READ")
    {
        if (!node->children.empty())
        {
            ...
            if (!target.isLValue)
                reportError(node->children[0], "readln exige uma variavel ou posicao de vetor");
    ...
    if (node->label == "RETURN")
    {
        ...
        ExprResult value;
        if (hasValue)
            value = analyzeExpr(node->children[0]);
        ...
        if (hasValue)
            emitter.emit("return " + value.place);
        else
            emitter.emit("return");
        return;
    }
...
ExprResult SemanticAnalyzer::resolveLValue(ASTNode *node)
{
    ...
    if (node->label == "ARRAY")
        return analyzeArrayAccess(node);
...
ExprResult SemanticAnalyzer::analyzeArrayAccess(ASTNode *node)
{
    ...
    if (!index.type.isError() && index.type.name != "int")
        reportError(node->children[1], "indice de vetor deve ser inteiro");
...
ExprResult SemanticAnalyzer::analyzeCall(ASTNode *node)
{
    ...
    {
        reportError(node, "numero incorreto de argumentos em '" + callee + "'");
    }
    for (const auto &arg : args)
        emitter.emit("param " + arg.place);
...
ExprResult SemanticAnalyzer::analyzeBinary(ASTNode *node, const string &op)
{
    ...
    if (op == "==" || op == "!=")
    {
        if (!comparable(left.type, right.type))
            reportError("erro semantico: comparacao de igualdade com tipos incompativeis");
        emitter.emit(temp + " = " + left.place + " " + op + " " + right.place);
        return {TypeInfo::Named("bool"), temp, false};
    }
    if (op == "<" || op == "<=" || op == ">" || op == ">=")
    {
        if (!left.type.isNumeric() || !right.type.isNumeric())
            reportError("erro semantico: comparacao relacional exige operandos numericos");
        emitter.emit(temp + " = " + left.place + " " + op + " " + right.place);
        return {TypeInfo::Named("bool"), temp, false};
    }
    if (op == "+" || op == "-" || op == "*" || op == "/" || op == "%")
    {
        if (!left.type.isNumeric() || !right.type.isNumeric())
            reportError("erro semantico: operador aritmetico exige operandos numericos");
        emitter.emit(temp + " = " + left.place + " " + op + " " + right.place);
...
ExprResult SemanticAnalyzer::analyzeAssignment(ASTNode *node)
{
    ...
    ExprResult right = analyzeExpr(node->children[1]);
    if (!left.isLValue)
        reportError("erro semantico: lado esquerdo da atribuicao nao e um lvalue");
    else if (!compatibleAssignment(left.type, right.type))
        reportError("erro semantico: atribuicao incompativel entre '" + left.type.toString() + "' e '" + right.type.toString() + "'");
...
ExprResult SemanticAnalyzer::analyzePrimary(ASTNode *node)
{
    ...
    if (startsWith(node->label, "LITERAL."))
        return {TypeInfo::Named("string"), '"' + suffixAfterDot(node->label) + '"', false};
    if (startsWith(node->label, "CHAR."))
        return {TypeInfo::Named("char"), '\'' + suffixAfterDot(node->label) + '\'', false};
    if (node->label == "TRUE" || node->label == "FALSE")
        return {TypeInfo::Named("bool"), lower(node->label), false};
    if (node->label == "CALL")
        return analyzeCall(node);
    if (node->label == "ARRAY")
        return analyzeArrayAccess(node);
    if (startsWith(node->label, "BOOLEAN_OP.") || startsWith(node->label, "RELATIONAL_OP.") || startsWith(node->label, "ADDITION_OP.") || startsWith(node->label, "MULTIPLICATION_OP."))
        return analyzeBinary(node, suffixAfterDot(node->label));
...
ExprResult SemanticAnalyzer::analyzeExpr(ASTNode *node)
{
    ...
    if (node->label == "ARRAY")
        return analyzeArrayAccess(node);
    if (node->label == "NOT" || node->label == "INC" || node->label == "DEC")
